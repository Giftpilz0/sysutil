...
func init() {
	rootCmd.AddCommand(waypipelauncherCmd)
	waypipelauncherCmd.Flags().StringVarP(&flagArguments, "arguments", "a", "", "Commandline arguments for waypipe")
}

var waypipelauncherCmd = &cobra.Command{
	Use:   "waypipelauncher",
	Args:  cobra.MaximumNArgs(0),
	Short: "Launch a waypipe connection using fuzzel",
...
func waypipeShowWofi(hosts map[string][]HostEntry) (string, HostEntry, error) {
	...
	cmd := exec.Command("fuzzel", "--prompt", "Waypipe hosts:", "--dmenu", "-i")
